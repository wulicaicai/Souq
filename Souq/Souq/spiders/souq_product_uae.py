...
class SouqSpider(scrapy.Spider):
    name = 'souq_product_uae'
    url = 'http://third.gets.com/api/index.php?act=getSouqCategory&sec=20171212getscn&site=1'
    # url = 'http://third.gets.com/api/index.php?act=getSouqCategory&sec=20171212getscn&site=1'
    # url2 = 'https://saudi.souq.com'
    start_spider_url = []
    start_spider_path = []
    req = get(requests.session(), url)
    # res = json.loads(req.text.replace('﻿',''))   #replace 网页传递过来带了bom头,需要用replace去掉!
    ...
    def parse(self, response):
        ...
        with open('souq_prjduct_uae.txt', 'a', encoding='utf-8') as f:
